...
addpath('/Users/Zhenjie/Github/EoS_Remastered/Library');
addpath('/Users/Zhenjie/Github/IdealFermiEOS');
%% Define the cloud
mu0_hz=5000; %mu(z=0) at center of cloud in unit of kHz
Ttrap_khz=400; % Temperature of the cloud in kHz
mu0=mu0_hz*hh;
Ttrap=Ttrap_khz*hh;
r0=60e-6; %The trap radius at z=0 in microron
angle=5.5/180*pi; %The angle of the cone
...
%%
[ KappaTildeP, PTildeP, TTildeP, ~ , beta_mu_P ,~ ] = IdealFermiEOS();
%% get the x,y profile at z=0
[X0,Y0]=meshgrid(x,y);
R0=(X0.^2+Y0.^2).^(1/2);
Vr0=Vr(R0,r0);
mu=mu0-Vr0;
TTilde0=interp1(beta_mu_P,TTildeP,mu/Ttrap,'spline');
...
for i=1:512
    ...
    Vrz=Vr(Rz,r0z(i));
    mu=muz(i)-Vrz;
    TTildez=interp1(beta_mu_P,TTildeP,mu/Ttrap,'spline');
