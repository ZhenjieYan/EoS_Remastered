...
kB=1.380e-23;
Nsat=inf;
load('/Users/Zhenjie/Data/Processed/2016-09-08/2016-09-08Set2.mat')
warning ('off','all')
% %%
% [Pt,Kt,nsort,Vsort,Zsort,Ptsel,Ktsel,EFS1,P,zcor,Vsel]=EOS_Online( imglistS1Final{1},...
%     'ShowOutline',1,'KappaMode',2,'PolyOrder',10,'VrangeFactor',5,'IfHalf',0,'kmax',0.9,'kmin',0.15,...
%     'Fudge',Fudge,'BGSubtraction',0,'IfFitExpTail',1,'Nsat',Nsat,'ShowPlot',1,'CutOff',inf,'IfHalf',0,'pixellength',pixellength,'SM',3,'IfBin',0,'BinGridSize',150,'IfCleanImage',1,'OutlineExtrapolate',1);
% 
...
for i=1:length(imglistS1Final)
    disp(i);
    [Pt,Kt,nsort,Vsort,Zsort,Ptsel,Ktsel,EFS1,P,zcor,Vsel]=EOS_Online( imglistS1Final{i},'ROI1',[700,800,1310,1820],...
    'ROI2',[700,1195,1310,1460],'TailRange',[1000,1650],'ShowOutline',0,'KappaMode',2,'PolyOrder',10,'VrangeFactor',5,'IfHalf',0,'kmax',0.9,'kmin',0.15,...
    'Fudge',Fudge,'BGSubtraction',0,'IfFitExpTail',0,'Nsat',Nsat,'ShowPlot',0,'CutOff',inf,'IfHalf',0,'pixellength',pixellength,'SM',3,'IfBin',0,'BinGridSize',150,'IfCleanImage',1,'OutlineExtrapolate',1);
...
for i=1:length(imglistS2Final)
    disp(i);
    [Pt,Kt,nsort,Vsort,Zsort,Ptsel,Ktsel,EFS1,P,zcor,Vsel]=EOS_Online( imglistS2Final{i},'ROI1',[700,800,1310,1820],...
    'ROI2',[700,1275,1310,1370],'TailRange',[1000,1650],'ShowOutline',0,'KappaMode',2,'PolyOrder',10,'VrangeFactor',5,'IfHalf',0,'kmax',0.9,'kmin',0.15,...
    ...
    'IfCleanImage',1,'OutlineExtrapolate',1);%
...
[KappaTildeS1BinV,~]=FiniteD( VBinV,VBinV*0,EFS1BinV,EFS1BinV*0,4);
...
for i=1:Iteration*length(ZS1sortlist)
    ...
    Nbin=round(rand()*200+200);
    Zgrid=sign(linspace(sign(min(ZS1sortlist{j}))*min(ZS1sortlist{j}).^2,sign(max(ZS1sortlist{j}))*max(ZS1sortlist{j}).^2,Nbin+1)).*sqrt(abs(linspace(sign(min(ZS1sortlist{j}))*min(ZS1sortlist{j}).^2,sign(max(ZS1sortlist{j}))*max(ZS1sortlist{j}).^2,Nbin+1)));
...
%%
Nbin=200;
Zgrid1=linspace(-(250^2),250^2,300+1);
...
figure1 = figure;
axes1 = axes('Parent',figure1,'unit','inch','position',[1,1,1.4,0.6]);
...
set(axes1,'XColor',[0 0 0],'YColor',[0 0 0],'ZColor',[0 0 0],'Ytick',[0 0.15 0.3],'Xtick',[-200,-100,0,100,200])
...
%plot(Zfit,KappaFit,'linewidth',1,'color','k');
