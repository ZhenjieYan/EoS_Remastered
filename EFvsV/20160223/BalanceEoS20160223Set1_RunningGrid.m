...
load('BGimg0223');
warning ('off','all');
filelistS1Final={'02-23-2016_22_49_00_top';'02-23-2016_22_47_19_top';'02-23-2016_22_45_38_top'};
filelistS2Final={'02-23-2016_22_49_50_top';'02-23-2016_22_48_10_top';'02-23-2016_22_46_29_top';};
...
for i=1:length(filelistS1Final)
    imgtemp=fitsread(['/Users/Zhenjie/Data/2016-02-23/',filelistS1Final{i},'.fits']);
    imglistS1Final=[imglistS1Final;imgtemp];
end
for i=1:length(filelistS2Final)
    imgtemp=fitsread(['/Users/Zhenjie/Data/2016-02-23/',filelistS2Final{i},'.fits']);
    imglistS2Final=[imglistS2Final;imgtemp];
end
...
for i=1:length(imglistS1Final)
    disp(i);
    [Pt,Kt,nsort,Vsort,Zsort,Ptsel,Ktsel,EFS1,P,zcor,Vsel]=EOS_Online( imglistS1Final{i},'ROI1',[215,25,312,402],...
    'ROI2',[209,187,335,243],'TailRange',[65,345],'ShowOutline',0,'KappaMode',2,'PolyOrder',10,'VrangeFactor',5,'IfHalf',0,'kmax',0.9,'kmin',0.15,...
    'Fudge',Fudge,'BGSubtraction',BGimg,'IfFitExpTail',0,'Nsat',Nsat,'ShowPlot',0,'CutOff',inf,'IfHalf',0,'pixellength',pixellength,'SM',3,'IfBin',0,'BinGridSize',150);
...
for i=1:length(imglistS1Final)
    disp(i);
    [Pt,Kt,nsort,Vsort,Zsort,Ptsel,Ktsel,EFS2,P,zcor,Vsel]=EOS_Online( imglistS2Final{i},'ROI1',[215,25,312,402],...
    'ROI2',[209,187,335,243],'TailRange',[85,325],'ShowOutline',0,'KappaMode',2,'PolyOrder',10,'VrangeFactor',5,'IfHalf',0,'kmax',0.9,'kmin',0.15,...
    'Fudge',Fudge,'BGSubtraction',BGimg,'IfFitExpTail',0,'Nsat',Nsat,'ShowPlot',0,'CutOff',inf,'IfHalf',0,'pixellength',pixellength,'SM',3,'IfBin',0,'BinGridSize',150);
...
for i=1:Iteration*length(ZS1sortlist)
    ...
    %Zgrid=sign(linspace(sign(min(Z_vec))*min(Z_vec).^2,sign(max(Z_vec))*max(Z_vec).^2,Nbin+1)).*sqrt(abs(linspace(sign(min(Z_vec))*min(Z_vec).^2,sign(max(Z_vec))*max(Z_vec).^2,Nbin+1)));
...
ZList=ZList/1e-6;
%%
plot(ZList,KappaList,'r.','markersize',2)
ylim([-1,3.5]);
...
[ZBinK,KappaBinK,~,~,~,KappaBinErrK]=BinGrid(ZList,KappaList,Zgrid,1);
