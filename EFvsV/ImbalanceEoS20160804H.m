...
mask=isnan(nS1Bin);nS1Bin(mask)=[];ZS1BinZ(mask)=[];
mask=isnan(nS2Bin);nS2Bin(mask)=[];ZS2BinZ(mask)=[];
...
mask=VBinV>Vth;
EFS1BinV=EFS1BinV-mean(EFS1BinV(mask));
EFS2BinV=EFS2BinV-mean(EFS2BinV(mask));
...
P=IdealGasFit( Vfit,nS1fit,[2000,250,1] );
P(3)
%%
...
plot(VBinV,n_ideal_BinV+0.615*nS2BinV,'g-','linewidth',1,'DisplayName','ENS EoS, A=-0.615');
hold off
xlim([0,5000])
legend show
ylabel('n (um^{-3})');
...
%%
save('Result0804H.mat','T_trap','mu_trap_S1','VBinV','nS1BinV','nS2BinV','KappaTildeS1BinV','KappaTildeS2BinV')